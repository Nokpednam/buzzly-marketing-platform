...
export async function seedDemoDataForWorkspace(workspaceId: string, supabase: SupabaseClient) {
  try {
    ...
    // 1. Check if they already have insights (to avoid infinite loops or double seeding)
    const { count, error: countError } = await supabase
      .from("ad_insights")
      .select("*", { count: "exact", head: true })
      .eq("team_id", workspaceId); // Wait, ad_insights doesn't have team_id? Let's fetch campaigns first.
    ...
    // 2. Create Ad Accounts if none exist
    ...
    if (!adAccounts || adAccounts.length === 0) {
      const { data: newAccounts } = await supabase
        .from("ad_accounts")
        .insert([
          { team_id: workspaceId, platform_id: "facebook", account_id: "act_fb_demo", account_name: "Facebook Ads (Demo)", status: "ACTIVE" },
          { team_id: workspaceId, platform_id: "tiktok", account_id: "act_tt_demo", account_name: "TikTok Ads (Demo)", status: "ACTIVE" }
        ])
        .select();
    ...
    // 3. Create Campaigns if none exist
    let campaigns = existingCampaigns;
    if (!campaigns || campaigns.length === 0) {
      const fbAcc = adAccounts.find(a => a.platform_id === "facebook");
      const ttAcc = adAccounts.find(a => a.platform_id === "tiktok");
