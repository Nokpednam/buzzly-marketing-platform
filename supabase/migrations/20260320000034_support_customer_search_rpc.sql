-- ============================================================================
-- Migration: Support Customer Search RPC
-- Timestamp: 20260320000034
--
-- Problem:  Search in Tier Management doesn't work:
--           1. PostgREST ilike filter may need * instead of % (URL encoding)
--           2. profile_customers has no email — can't search by email
--           3. RLS + complex filter may cause empty results
--
-- Fix:      Create RPC search_customers_for_support(p_query text) that:
--           - Joins profile_customers + customer + loyalty_points
--           - Searches first_name, last_name, email, user_id
--           - Employee-only, returns CustomerSearchResult shape
-- ============================================================================
...
BEGIN
    ...
    WHERE
        ...
        OR (c.email IS NOT NULL AND c.email ILIKE p_query)
...
COMMENT ON FUNCTION public.search_customers_for_support(text) IS
    'Employee-only search for Tier Management. Searches first_name, last_name, email, user_id.';
