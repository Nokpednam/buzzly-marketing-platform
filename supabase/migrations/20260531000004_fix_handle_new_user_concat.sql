-- ============================================================================
-- Migration: Fix handle_new_user operator precedence bug
-- Timestamp: 20260531000004
--
-- Fixes an issue where `new.raw_user_meta_data->>'first_name' || ' ' || new.raw_user_meta_data->>'last_name'`
-- threw an "operator does not exist: text ->> unknown" error because of
-- PostgreSQL's operator precedence evaluating `||` before `->>`.
-- ============================================================================
...
COMMENT ON FUNCTION public.handle_new_user() IS
    'Trigger on auth.users INSERT. Creates customer/profile_customers/loyalty_points for customers. Does NOT auto-create workspace — customers must create it manually. Fixed operator precedence issue.';
