...
BEGIN
    ...
    -- Safe Insert for Teams
    WITH teams_data(id, name, description, logo_url, workspace_url, status, owner_id, business_type_slug, industry_slug) AS (
      VALUES
    ...
    INSERT INTO public.teams (id, name, description, logo_url, workspace_url, status, owner_id, business_type_id, industries_id)
    SELECT
    ...
    -- IDs start with 60..
    -- Safe Insert for Workspaces
    WITH workspace_data(id, team_id, workspace_name, status, business_type_slug, industry_slug) AS (
      VALUES
        ('60000000-0000-0000-0000-000000000001'::uuid, '50000000-0000-0000-0000-000000000001'::uuid, 'Buzzly Workspace', 'active', 'agency', 'digital-marketing'),
        ('60000000-0000-0000-0000-000000000002'::uuid, '50000000-0000-0000-0000-000000000002'::uuid, 'TechStart Workspace', 'active', 'technology', 'software-development'),
        ('60000000-0000-0000-0000-000000000003'::uuid, '50000000-0000-0000-0000-000000000003'::uuid, 'ShopMax Workspace', 'active', 'e-commerce', 'retail'),
        ('60000000-0000-0000-0000-000000000004'::uuid, '50000000-0000-0000-0000-000000000004'::uuid, 'HealthPlus Workspace', 'active', 'healthcare', NULL),
        ('60000000-0000-0000-0000-000000000005'::uuid, '50000000-0000-0000-0000-000000000005'::uuid, 'EduLearn Workspace', 'active', 'education', NULL),
        ('60000000-0000-0000-0000-000000000006'::uuid, '50000000-0000-0000-0000-000000000006'::uuid, 'Foodies Workspace', 'active', 'food-beverage', 'hospitality'),
        ('60000000-0000-0000-0000-000000000007'::uuid, '50000000-0000-0000-0000-000000000007'::uuid, 'PropertyHub Workspace', 'active', 'real-estate', NULL),
        ('60000000-0000-0000-0000-000000000008'::uuid, '50000000-0000-0000-0000-000000000008'::uuid, 'FastLogistics Workspace', 'active', 'e-commerce', 'logistics')
    )
    INSERT INTO public.workspaces (id, team_id, workspace_name, status, business_type_id, industries_id)
    SELECT
      w.id, w.team_id, w.workspace_name, w.status,
      bt.id, ind.id
    FROM workspace_data w
    JOIN public.business_types bt ON bt.slug = w.business_type_slug
    LEFT JOIN public.industries ind ON ind.slug = w.industry_slug
    -- Ensure the team exists to avoid FK error
    WHERE EXISTS (SELECT 1 FROM public.teams t WHERE t.id = w.team_id)
    ON CONFLICT (id) DO NOTHING;

    -- 6. Insert Workspace Members
    -- IDs start with 70..
    -- Safe Insert for Workspace Members (Owners)
    INSERT INTO public.workspace_members (id, workspace_id, user_id, role_customer_id, status, joined_at)
    SELECT
      val.id::uuid, val.workspace_id::uuid, v_owner_id, val.role_id::uuid, val.status, (NOW() - val.interval_offset::interval)
    FROM (VALUES
      ('70000000-0000-0000-0000-000000000001', '60000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', 'active', '6 months'),
      ('70000000-0000-0000-0000-000000000002', '60000000-0000-0000-0000-000000000002', '10000000-0000-0000-0000-000000000001', 'active', '4 months'),
      ('70000000-0000-0000-0000-000000000003', '60000000-0000-0000-0000-000000000003', '10000000-0000-0000-0000-000000000001', 'active', '3 months'),
      ('70000000-0000-0000-0000-000000000004', '60000000-0000-0000-0000-000000000004', '10000000-0000-0000-0000-000000000001', 'active', '5 months'),
      ('70000000-0000-0000-0000-000000000005', '60000000-0000-0000-0000-000000000005', '10000000-0000-0000-0000-000000000001', 'active', '2 months'),
      ('70000000-0000-0000-0000-000000000006', '60000000-0000-0000-0000-000000000006', '10000000-0000-0000-0000-000000000002', 'active', '1 month'),
      ('70000000-0000-0000-0000-000000000007', '60000000-0000-0000-0000-000000000007', '10000000-0000-0000-0000-000000000001', 'active', '7 months'),
      ('70000000-0000-0000-0000-000000000008', '60000000-0000-0000-0000-000000000008', '10000000-0000-0000-0000-000000000001', 'active', '2 weeks')
    ) AS val(id, workspace_id, role_id, status, interval_offset)
    WHERE EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = val.workspace_id::uuid)
    ON CONFLICT (id) DO NOTHING;

    -- 7. Insert Invitations
    -- Safe Insert for Invitations
    INSERT INTO public.workspace_members (id, workspace_id, invitation_email, role_customer_id, status, invitation_expires_at, created_at)
    SELECT
      val.id::uuid, val.workspace_id::uuid, val.email, val.role_id::uuid, val.status, (NOW() + val.expire_offset::interval), (NOW() - val.created_offset::interval)
    FROM (VALUES
      ('70000000-0000-0000-0000-000000000009', '60000000-0000-0000-0000-000000000001', 'newmarketer@gmail.com', '10000000-0000-0000-0000-000000000003', 'pending', '7 days', '2 days'),
      ('70000000-0000-0000-0000-000000000010', '60000000-0000-0000-0000-000000000001', 'designer.pro@email.com', '10000000-0000-0000-0000-000000000004', 'pending', '5 days', '4 days'),
      ('70000000-0000-0000-0000-000000000011', '60000000-0000-0000-0000-000000000002', 'developer@techcompany.com', '10000000-0000-0000-0000-000000000002', 'pending', '6 days', '1 day'),
      ('70000000-0000-0000-0000-000000000012', '60000000-0000-0000-0000-000000000003', 'inventory.manager@shop.com', '10000000-0000-0000-0000-000000000003', 'pending', '4 days', '3 days')
    ) AS val(id, workspace_id, email, role_id, status, expire_offset, created_offset)
    WHERE EXISTS (SELECT 1 FROM public.workspaces w WHERE w.id = val.workspace_id::uuid)
    ON CONFLICT (id) DO NOTHING;
...
WHERE EXISTS (SELECT 1 FROM public.teams t WHERE t.id = d.team_id)
ON CONFLICT (id) DO NOTHING;
