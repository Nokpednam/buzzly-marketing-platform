...
-- ===== Platforms =====
INSERT INTO platforms (id, name, slug, description, is_active) VALUES
  ('b1a00001-0000-0000-0000-000000000001', 'Facebook', 'facebook', 'Meta Facebook Ads Platform', true),
  ('b1a00002-0000-0000-0000-000000000002', 'Instagram', 'instagram', 'Meta Instagram Ads Platform', true),
  ('b1a00003-0000-0000-0000-000000000003', 'TikTok', 'tiktok', 'TikTok for Business Ads', true),
  ('b1a00004-0000-0000-0000-000000000004', 'Shopee', 'shopee', 'Shopee Marketplace Ads', true),
  ('b1a00005-0000-0000-0000-000000000005', 'Google', 'google', 'Google Ads Platform', true),
  ('b1a00006-0000-0000-0000-000000000006', 'LINE', 'line', 'LINE Ads Platform', true),
  ('b1a00007-0000-0000-0000-000000000007', 'YouTube', 'youtube', 'YouTube Ads via Google', true),
  ('b1a00008-0000-0000-0000-000000000008', 'Twitter/X', 'twitter', 'X (Twitter) Ads', true)
ON CONFLICT (id) DO UPDATE SET 
  name = EXCLUDED.name,
  description = EXCLUDED.description;
...
INSERT INTO external_api_status (id, platform_id, last_status_code, latency_ms, color_code, created_at) VALUES
  ('ea500001-0000-0000-0000-000000000001', 'b1a00001-0000-0000-0000-000000000001', 200, 145, '#10B981', NOW()),
  ('ea500002-0000-0000-0000-000000000002', 'b1a00002-0000-0000-0000-000000000002', 200, 132, '#10B981', NOW()),
  ('ea500003-0000-0000-0000-000000000003', 'b1a00003-0000-0000-0000-000000000003', 200, 289, '#10B981', NOW()),
  ('ea500004-0000-0000-0000-000000000004', 'b1a00004-0000-0000-0000-000000000004', 200, 178, '#10B981', NOW()),
  ('ea500005-0000-0000-0000-000000000005', 'b1a00005-0000-0000-0000-000000000005', 200, 95, '#10B981', NOW()),
  ('ea500006-0000-0000-0000-000000000006', 'b1a00006-0000-0000-0000-000000000006', 200, 156, '#10B981', NOW()),
  ('ea500007-0000-0000-0000-000000000007', 'b1a00007-0000-0000-0000-000000000007', 200, 112, '#10B981', NOW()),
  ('ea500008-0000-0000-0000-000000000008', 'b1a00008-0000-0000-0000-000000000008', 429, 2450, '#EF4444', NOW())
ON CONFLICT (id) DO UPDATE SET 
...
INSERT INTO social_posts (id, platform_id, post_type, content, status, scheduled_at, published_at, impressions, reach, likes, comments, shares, saves, clicks, engagement_rate, created_at) VALUES
  (gen_random_uuid(), 'b1a00001-0000-0000-0000-000000000001', 'feed', '🚀 Exciting news! We''ve just launched our new AI-powered analytics dashboard. Check it out and let us know what you think! #MarketingTech #AI #Analytics', 'published', NOW() - INTERVAL '6 months', NOW() - INTERVAL '6 months', 45000, 38000, 1250, 89, 234, 120, 890, 4.5, NOW() - INTERVAL '6 months'),
  (gen_random_uuid(), 'b1a00002-0000-0000-0000-000000000002', 'story', '📊 Behind the scenes: How our team builds data-driven marketing strategies. Swipe up to learn more! #MarketingTips', 'published', NOW() - INTERVAL '5 months', NOW() - INTERVAL '5 months', 32000, 28000, 890, 45, 67, 89, 456, 3.8, NOW() - INTERVAL '5 months'),
  (gen_random_uuid(), 'b1a00003-0000-0000-0000-000000000003', 'feed', '💡 Pro tip: Always test your ad creatives with at least 3 variations. Our data shows this improves CTR by 40%! #DigitalMarketing', 'published', NOW() - INTERVAL '4 months', NOW() - INTERVAL '4 months', 67000, 55000, 2100, 156, 345, 200, 1200, 5.2, NOW() - INTERVAL '4 months'),
  (gen_random_uuid(), 'b1a00001-0000-0000-0000-000000000001', 'feed', 'Summer sale is LIVE! 30% off all annual plans 🌴', 'published', NOW() - INTERVAL '3 months', NOW() - INTERVAL '3 months', 29000, 24000, 423, 56, 89, 40, 610, 3.8, NOW() - INTERVAL '3 months'),
  (gen_random_uuid(), 'b1a00002-0000-0000-0000-000000000002', 'story', 'Quick tip: Always A/B test your ad creatives! Here''s why... 🧪', 'published', NOW() - INTERVAL '2 months', NOW() - INTERVAL '2 months', 21000, 17000, 567, 78, 45, 62, 330, 4.2, NOW() - INTERVAL '2 months'),
  (gen_random_uuid(), 'b1a00008-0000-0000-0000-000000000008', 'feed', 'We just hit 100K users! Thank you for being part of our journey 🎉', 'published', NOW() - INTERVAL '1 month', NOW() - INTERVAL '1 month', 48000, 39000, 1234, 189, 312, 140, 980, 5.1, NOW() - INTERVAL '1 month'),
  (gen_random_uuid(), 'b1a00002-0000-0000-0000-000000000002', 'feed', 'New integration coming soon! Stay tuned for the big reveal 👀', 'scheduled', NOW() + INTERVAL '2 days', NULL, NULL, NULL, 0, 0, 0, 0, 0, NULL, NOW() - INTERVAL '1 day'),
  (gen_random_uuid(), 'b1a00001-0000-0000-0000-000000000001', 'feed', 'Black Friday special announcement coming this Friday!', 'scheduled', NOW() + INTERVAL '3 days', NULL, NULL, NULL, 0, 0, 0, 0, 0, NULL, NOW()),
  (gen_random_uuid(), 'b1a00006-0000-0000-0000-000000000006', 'feed', 'LINE Official Account now connected! 🇹🇭', 'published', NOW() - INTERVAL '15 days', NOW() - INTERVAL '15 days', 17000, 14000, 289, 34, 12, 22, 260, 3.0, NOW() - INTERVAL '15 days'),
  (gen_random_uuid(), 'b1a00006-0000-0000-0000-000000000006', 'feed', 'เปิดตัวฟีเจอร์ใหม่! วิเคราะห์ข้อมูลด้วย AI 🤖', 'published', NOW() - INTERVAL '20 days', NOW() - INTERVAL '20 days', 25000, 21000, 456, 67, 23, 44, 420, 3.7, NOW() - INTERVAL '20 days')
ON CONFLICT DO NOTHING;
...
-- ===== Priority Levels =====
INSERT INTO priority_level (id, level_name, description, color_code) VALUES
...
INSERT INTO product_categories (id, name, slug, description, display_order) VALUES
  ('b4500001-0000-0000-0000-000000000001', 'Subscription Plans', 'subscriptions', 'Monthly and yearly subscription plans', 1),
  ('b4500002-0000-0000-0000-000000000002', 'Add-ons', 'addons', 'Additional features and services', 2),
  ('b4500003-0000-0000-0000-000000000003', 'Credits', 'credits', 'Platform credits for additional usage', 3),
  ('b4500004-0000-0000-0000-000000000004', 'Enterprise', 'enterprise', 'Custom enterprise solutions', 4)
ON CONFLICT (id) DO UPDATE SET 
  name = EXCLUDED.name,
  description = EXCLUDED.description;
